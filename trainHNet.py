...
# data io
train_file = data_base_path + 'H-Net_Data_lwq/train/'
valid_file = data_base_path + 'H-Net_Data_lwq/test/'
test_file = data_base_path + 'H-Net_Data_lwq/test/'
io = tfio(BATCH_SIZE, BATCH_SIZE, 200, DATA_AUG, width, height, channel, train_file, valid_file, test_file, 1001, 1)
...
network = H_Net(photo_pl, photo_flat, uds_pl, uds_flat, label_pl, is_training_pl, keep_prob_pl, VSIZE, OPTIMIZER,
                MOMENTUM, learning_rate, global_step, logout)
...
def eval_net(sess, batch_num, batch, batch_size, acc, _predict):
    ...
    for index in range(batch_num):
        ...
        ac, r = sess.run([acc, _predict],
                         feed_dict = {photo_pl:p, uds_pl:u, label_pl:l, keep_prob_pl:1.0, is_training_pl:False})
...
def train():
    n_epoch = NEPOCH
    train_p_batch, train_u_batch, train_l_batch = io.get_onebatch_traindata(width = width, height = height)
    valid_p_batch, valid_u_batch, valid_l_batch = io.get_onebatch_validdata(width = width, height = height)
    # test_p_batch, test_u_batch, test_l_batch = io.get_onebatch_testdata(width = width, height = height)
    with tf.Session(config = config) as sess:
        ...
        train_best_ac, val_best_ac = 0, 0
        # test_best_ac = 0
        
        model_path = os.path.join(BASE_DIR, LOGDIR + '/model-' + MPX + '-' + NET_NAME + '-' + VERSION)
        ckpt = tf.train.get_checkpoint_state(model_path)
        if ckpt and ckpt.model_checkpoint_path:
            model_saver = tf.train.Saver(var_list = var_list)
            model_saver.restore(sess, ckpt.model_checkpoint_path)
            train_start_epoch = int(ckpt.model_checkpoint_path.split('-epoch-')[0].split('/')[-1]) + 1
            if BGEPOCH > train_start_epoch: train_start_epoch = BGEPOCH
            val_best_ac = float(ckpt.model_checkpoint_path.split('-' + MPX + '-')[2].split('.ckpt')[0])
            logout(('\n*** %s model is loaded ***\n' % ckpt.model_checkpoint_path))
        else:
            train_start_epoch = 0
        
        train_step = one_epoch_step * train_start_epoch
        
        val_saver = tf.train.Saver(var_list = var_list, max_to_keep = 1)
        
        train_writer = tf.summary.FileWriter(os.path.join(BASE_DIR, LOGDIR + NET_NAME + '-' + VERSION + '/train'),
                                             sess.graph)
        valid_writer = tf.summary.FileWriter(os.path.join(BASE_DIR, LOGDIR + NET_NAME + '-' + VERSION + '/valid'),
                                             sess.graph)
        
        logout('train files\t\t:{:d}'.format(io.train_num))
        logout('valid files\t\t:{:d}'.format(io.valid_num))
        logout('test files\t\t:{:d}'.format(io.test_num))
        logout('one_epoch_step\t:{:d}'.format(one_epoch_step))
        logout('start epoch\t\t:{:d}'.format(train_start_epoch))
        logout('val_best_ac\t\t:{}'.format(val_best_ac))
        
        for epoch in range(train_start_epoch, n_epoch):
            start_time = time.time()
            
            logout('*** new epoch with learning_rate:{:f} ***'.format(sess.run(learning_rate)))
            for step in range(one_epoch_step):
                batchphoto, batchuds, batchlabel = sess.run([train_p_batch, train_u_batch, train_l_batch])
                
                sess.run([network.ae_train_op, network.metric_train_op],
                         feed_dict = {photo_pl:batchphoto, uds_pl:batchuds, label_pl:batchlabel, keep_prob_pl:1.0,
                                      is_training_pl:True})
                
                train_step += 1
                if train_step % PRINT_FREQ == 0:
                    gen_loss_tr, hinge_loss_tr, summ_train = sess.run(
                        [network.gen_loss, network.hinge_loss, network.summary],
                        feed_dict = {photo_pl:batchphoto, uds_pl:batchuds, label_pl:batchlabel, keep_prob_pl:1.0,
                                     is_training_pl:False})
                    
                    batchphoto_v, batchuds_v, batchlabel_v = sess.run([valid_p_batch, valid_u_batch, valid_l_batch])
                    summ_valid = sess.run(network.summary,
                                          feed_dict = {photo_pl:batchphoto_v, uds_pl:batchuds_v, label_pl:batchlabel_v,
                                                       keep_prob_pl:1.0, is_training_pl:False})
                    
                    train_writer.add_summary(summ_train, train_step)
                    valid_writer.add_summary(summ_valid, train_step)
                    
                    logout('{:s}\t'
                           'Epoch {:d} Step {:d}\t'
                           'Train: gen_loss:{:f} hinge_loss:{:f}'.format(
                        time.strftime('%m-%d %H:%M:%S', time.localtime(time.time())), epoch, step + 1, gen_loss_tr,
                        hinge_loss_tr))
            
            test_start_time = time.time()
            # calculate valid performance
            val_ac = eval_net(sess, io.get_valid_itr(), [valid_p_batch, valid_u_batch, valid_l_batch], io.batch_size_v,
                              network.metricacc, network.predict)
            
            if val_ac > val_best_ac:
                # if found better model calculate test accuracy
                val_best_ac = val_ac
                val_saver.save(sess, str('%s/%s/model-%s-%s-%s/%d-epoch-val-%.5f.ckpt' % (
                    BASE_DIR, LOGDIR, MPX, NET_NAME, VERSION, epoch, val_best_ac)))
                # # calculate test performance
                # test_ac = eval_net(io.get_test_itr(), [test_p_batch, test_u_batch, test_l_batch], io.batch_size_te, network.metricacc, network.predict)
                # if test_ac > test_best_ac: test_best_ac = test_ac
                # logout("test:\tnum batch:{:d}\taccuracy:{:f}\tbest:{:f}".format(1, test_ac, test_best_ac))
            logout("test tooks %f min \t best val ac %f" % ((time.time() - test_start_time) / 60, val_best_ac))
            logout('******Epoch %d of %d Took %f min******\n' % (epoch, n_epoch, (time.time() - start_time) / 60))
...
def eval_network():
    ...
    with tf.Session(config = config) as sess:
        ...
        test_start_time = time.time()
        eval_net(sess, io.get_valid_itr(), [valid_p_batch, valid_u_batch, valid_l_batch], io.batch_size_v,
                 network.metricacc, network.predict)
